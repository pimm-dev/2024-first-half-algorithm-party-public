...
#define M_MAX 12
#define D_MIN 1
#define ERR_ENABLE_QUERY_NOT_BE_ZERO "Enable query flag must not be zero."
...
int main(int argc, char* argv[]) {
  ...
  int p = stoi(argv[1]), n = stoi(argv[2]), enable_query = stoi(argv[3]);
  if (enable_query == 0) {
    cout << ERR_ENABLE_QUERY_NOT_BE_ZERO << endl;
    exit(1);
  }
  validate_arg(p, n);
  cout << p << ' ' << n << '\n';
  ...
  for (int _i = 0; _i < n; _i++) {
    oper = gen_random(1, 3);
    int oper_flag = enable_query & (1 << (oper - 1));
    while (oper_flag != (1 << (oper - 1))) {
        oper = gen_random(1, 3);
        oper_flag = enable_query & (1 << (oper - 1));
    }
    switch (oper)
    {
    case 1:
